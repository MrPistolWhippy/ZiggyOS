...
typedef struct { uint32_t core_id; const char* lifecycle_state; uint32_t total_cycles; uint32_t active_cycles; } core_lifecycle_t;
...
int main() {
    ...
    acoustic_packet_t spike = { 44100, 16, 18500 }; uint8_t scr_b = 0; z_acoustic_bootstrap_decode(&spike, &scr_b);
    acoustic_packet_t valid_sig = { 44100, 16, 12000 }; z_acoustic_bootstrap_decode(&valid_sig, &scr_b);
    z_acoustic_verify_parity(scr_b, 0xFF); printf("\n");
    for (int i = 0; i < MAX_CORES; i++) { core_lifecycle_matrix[i].core_id = i; core_lifecycle_matrix[i].lifecycle_state = "BOOT_INIT"; core_lifecycle_matrix[i].total_cycles = 0; core_lifecycle_matrix[i].active_cycles = 0; }
