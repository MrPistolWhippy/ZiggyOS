...
static void ziggy_mutex_lock(spinlock_t *l, uint32_t p) { uint32_t timeout=0; while(atomic_amoswap(&l->lock, 1)==1) { if(p>l->owner_priority && l->owner_priority!=0) l->owner_priority=p; usleep(1); if(++timeout > 5000) { __sync_lock_release(&l->lock); break; } } l->owner_priority=p; }
