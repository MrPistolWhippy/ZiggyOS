...
typedef struct { char name[16]; uint32_t start_block; uint32_t size_bytes; } zfs_dir_entry_t;
...
static void z_fs_init() {
    strcpy(current_dir[0].name, "."); current_dir[0].start_block = 1; current_dir[0].size_bytes = 0;
    strcpy(current_dir[1].name, ".."); current_dir[1].start_block = 1; current_dir[1].size_bytes = 0;
...
static void z_entropy_harvest() {
    uint32_t seed = 0xACE1U;
    for (int i = 0; i < ENTROPY_POOL_SZ; i++) {
        seed = (seed >> 1) ^ (-(seed & 1u) & 0xB400u);
        entropy_pool[i] = seed ^ (uint32_t)i;
    }
}
static uint32_t z_get_random() { entropy_ptr = (entropy_ptr + 1) % ENTROPY_POOL_SZ; return entropy_pool[entropy_ptr]; }
static void z_oaep_pad(const uint8_t* msg, uint32_t m_len, uint8_t* out_padded) {
    uint32_t mask = z_get_random(); memset(out_padded, 0, 32);
    memcpy(out_padded, msg, m_len > 16 ? 16 : m_len); out_padded[16] = 0x01;
    for (int i = 0; i < 32; i++) out_padded[i] ^= ((mask >> ((i % 4) * 8)) & 0xFF);
}
